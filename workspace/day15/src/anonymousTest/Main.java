...
//		InterA ia2 = new InterA(); //구현되지않은 메소드이므로 오류(추상메소드이므로)
		
		InterA ia = new InterA() { //업캐스팅으로 구현됌
		...
		//구현되지 않은 것을 구현 해주면 객체를 생성할 수 있다
			...
			public void printData() {
				...
				//ia객체는 로컬 클래스 안에있다(main안에 static활용했기때문)
			}
		};
...
//		인터페이스 객체 생성할 수 없어서 익명클래스를 통해 인터페이스의 
...
//		업캐스팅
//		익명 클래스의 인스턴스는 인터페이스 InterA타입의 변수에 저장된다(업캐스팅)
