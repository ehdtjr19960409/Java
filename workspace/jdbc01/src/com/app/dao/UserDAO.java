...
public class UserDAO {
	...
	public boolean checkId(String userId) {
		...
		String query = "SELECT user_number, user_id FROM tbl_user WHERE user_id = ?";
		...
		try {
			...
			//우리가 문자열로 작성한 쿼리가 rpeparedStatement에 들어가서 sql명령어로 사용된다
			//쿼리에 ?가 있다면(미완성상태) -> ?를 매개변수로 받은 userId로 바꿔야한다
			//preparedStatement의 기능으로 완성시켜준다
			//set자료형(물음표의 순서, 넣을 값);
			
			preparedStatement.setString(1, userId);
			
			//완성된 쿼리를 실행시키는 방법 2가지
			//1. executeQuery()
